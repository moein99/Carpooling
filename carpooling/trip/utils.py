...
class SpotifyAgent:
    ...
    def refresh_access_token(self):
        ...
        response = requests.post(url=self.token_api, data=data, headers=headers).json()
    ...
    def search_items(self, query, item_type):
        ...
        for item in response[item_type + 's']['items']:
            result.append(SpotifyAgent.get_item_json(item, item_type))
        return result

    @staticmethod
    def get_item_json(item, item_type):
        artists = ' '.join(SpotifyAgent.get_artists_name(item['artists']))
    ...
    def add_items(self, playlist_id, item_id, item_type):
        if item_type == ItemType.TRACK:
            self.spotify.user_playlist_add_tracks(SPOTIFY_USERNAME, playlist_id, [item_id])
        elif item_type == ItemType.ALBUM:
            self.spotify.user_playlist_add_tracks(SPOTIFY_USERNAME, playlist_id, self.get_album_tracks_ids(item_id))
